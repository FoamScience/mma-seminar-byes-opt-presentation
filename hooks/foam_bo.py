...
def foam_bo_step(self, cfg, context):
    ...
    opt_area = grid.get_area(opt, x_range=(2, 10), opacity=0.3, color=[self.important_color, self.important_color])
    arrows = [Arrow(dots[3].get_center()+3*UP+0.5*RIGHT, d.get_center(), color=self.warn_color, max_tip_length_to_length_ratio=0.04, max_stroke_width_to_length_ratio=0.3) for d in [dots[2], dots[4]]]
    self.play(FadeIn(opt_area, run_time=self.fadein_rt), AnimationGroup(*[GrowArrow(a, run_time=self.fadein_rt) for a in arrows]))
    tt = Text("generated by surrogate model", font_size=self.s_size, color=self.warn_color).next_to(arrows[0], UP)
    tt1 = Text("Best parameter set", font_size=self.s_size, color=self.warn_color).next_to(tt, 0.5*UP)
